...
def search_edb(
    ...
    Args:
        ...
        type_ (str, optional): The exploit type.
        Possible types are [dos, local, remote, shellcode, papers, webapps].
        Defaults to "".
        ...
        content (str, optional): The exploit content.
        Warning, may increase response time. Defaults to "".

        author (str, optional): The exploit's author. Defaults to "".

        tag (int | str, optional): The exploit's tag. Possible tags are\n
        [WordPress Core, Metasploit Framework (MSF),
        WordPress Plugin, SQL Injection (SQLi), Cross-Site Scripting (XSS),
        File Inclusion (LFI/RFI), Cross-Site Request Forgery (CSRF), Denial of Service (DoS),
        Code Injection, Command Injection, Authentication Bypass / Credentials Bypass (AB/CB),
        Client Side, Use After Free (UAF), Out Of Bounds, Remote, Local, XML External Entity (XXE),
        Integer Overflow, Server-Side Request Forgery (SSRF), Race Condition,
        NULL Pointer Dereference, Malware, Buffer Overflow, Heap Overflow, Type Confusion,
        Object Injection, Bug Report, Console, Pwn2Own, Traversal, Deserialization].\n
        Defaults to "".

        tag_verify (bool, optional): Make user choose between different tags
        if the tag string in argument is contained in multiple possible tags
        or checks if no tags were found with the user input. Defaults to True.

        verified (bool, optional): Search only verified / unverified exploits. Defaults to "".

        hasapp (bool, optional): Search only exploits that have
        or don't have a vulnerable application attached. Defaults to "".

        nomsf (bool, optional): Search only exploits that use
        or don't use Metasploit Framework. Defaults to "".

        print_ (bool, optional): Switch to print  a table with results. Defaults to False.

    Returns:
        list[Exploit]: Prints the list of exploits found if print_ is True
        and returns a list of Exploit objects.
    """
    ...
    url = f"https://www.exploit-db.com/search?q={title}&type={type_}\
            &platform={platform}&port={port}&text={content}&e_author={author}\
            &tag={tag}&verified={verified}&hasapp={hasapp}&nomsf={nomsf}"
...
def search_cve(cve: str) -> Optional[List[Exploit]]:
    """
    Searches exploit-db.com CVE.

    Args:
        cve (str): The CVE to search.
        The argument can be given in 2 different forms : CVE-1234-1234 or 1234-1234.
